    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            if min_loss > (test_loss / len(val_loader)):
                ...
                if decrease % 5 == 0:
                    print('saving model...')
                    torch.save(model.state_dict(), 'saved_model/ViT_UNet-{:.3f}.pt'.format(val_iou_score / len(val_loader)))
...
#model.load_state_dict(torch.load('saved_model/UTNet.pt',map_location=torch.device("cuda")))
#print("Loaded model.")
...
torch.save(model.state_dict(), "saved_model/ViT_UNet.pt")
...
with open("ViT_UNet.pkl", "wb") as tf:
